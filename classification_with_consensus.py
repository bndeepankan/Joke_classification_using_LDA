...
class Consensus:
    ...
    def find_predict(self):
        ...
        pred = []
        for ind in self.X.index:
            for topic in self.header:
                if self.X[topic][ind] > 0:
    ...
    def report_classification(self):
        ...
        """
        print('Classification by selecting features. Size of feature space is: {}'.format(self.X.shape[1]))
        print('==' * 30)
        print('Report of Consensus algorithm')
        print(metrics.classification_report(self.pred, self.y.to_numpy()))
